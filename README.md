...
4. Integrate Payment Reader: `NFC Rasberry Pi HAT`
5. Integrate Locking Mechanism: Connect a relay module to control the solenoid lock based on server commands.
6. Set Up Display: Add a touchscreen for user interaction, showing payment status.
7. Set Up Connectivity: Insert the 5G SIM card, configure the modem, and test internet connection.
8. Develop Server-Side Software: Set up your server for video processing, object detection, `Stripe API` integration, and command sending.
...
- `NFC Rasberry Pi HAT`: A NFC payment reader for accepting contactless payments (debit/credit cards, NFC, RFID), compatible with Raspberry Pi.
...
- `7" HDMI Touchscreen`: For user interface, showing payment status and product information, enhancing user experience. 7" HDMI Touchscreen.
...
- Database: To store transaction data, inventory levels, and machine status for remote management and data analysis.
...
1. Customer Initiates Transaction:
   ...
   - The customer presents their payment method (debit/credit card, `NFC`, or `RFID`) to the `NFC` device
...
3. Product Removal Detection:
   ...
   - The server runs the object detection model (e.g., `YOLO`) on the video feed to detect when a product is removed. Research suggests models like `YOLOv3-TinyE` can achieve up to 99.15% accuracy, as noted in *[A Design of Smart Unmanned Vending Machine for New Retail Based on Binocular Camera and Machine Vision](https://ieeexplore.ieee.org/document/9359457)*
4. Payment Capture:

   - Upon detection of product removal, the server captures the payment with Stripe for the exact amount of the taken product.
   - If the customer does not remove any product and closes the Vending Machine's door, the server voids the authorization after locking the door, ensuring no charge is made and the products are re-secured.
5. Locking Mechanism:
...
*Highlights trade-offs between accuracy, speed, and hardware complexity*
...
2. Set Up Raspberry Pi:
   ...
   - Install necessary software packages, including GStreamer for video streaming, FFmpeg for video processing, and the Stripe JavaScript SDK for payment processing.
3. Integrate Camera:
...
6. Set Up Display:
   ...
   - Develop a user interface using Python or JavaScript to generate QR codes, show payment status, product selection, and error messages, enhancing user experience.
...
8. Develop Server-Side Software:
   ...
   - Set up a local server to receive the video stream from the Raspberry Pi.
   - Implement object detection trained on images of your products, to detect when a product is removed.
